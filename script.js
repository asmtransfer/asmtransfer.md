...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (routesSection) {
    ...
    const from=calc.querySelector('#calc-from'),to=calc.querySelector('#calc-to'),price=calc.querySelector('#calc-price'),hint=calc.querySelector('#calc-placeholder'),wa=calc.querySelector('#calc-whatsapp'); [...new Set(routes.map(r=>r[0]))].forEach(c=>from.add(new Option(c,c))); const reset=()=>{price.textContent='—';hint.style.display='block';wa.href='https://wa.me/37377950129'}; from.onchange=()=>{to.innerHTML='<option value="">Выберите направление</option>';routes.filter(r=>r[0]===from.value).forEach(r=>to.add(new Option(r[1],r[1])));to.disabled=!from.value;reset()}; to.onchange=()=>{const r=routes.find(r=>r[0]===from.value&&r[1]===to.value);if(!r)return reset();price.textContent=r[2];hint.style.display='none';wa.href=`https://wa.me/37377950129?text=${encodeURIComponent(`Здравствуйте! Хочу заказать трансфер ASM Transfer: ${r[0]} → ${r[1]}. На сайте указана предварительная стоимость ${r[2]}. Если бронирование за 7 дней и более, прошу учесть скидку 10%. Подтвердите актуальную цену и возможность поездки.`)}`};
